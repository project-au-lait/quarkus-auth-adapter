...
class SpecLogFormatter implements LogFormatter {
  ...
  format_en(pageName: string, itemName: string, action: Action, input?: string): string {
    ...
    switch (action) {
      ...
      case Action.EXPECT_VISIBLE:
        ...
        break;
      case Action.EXPECT_NOT_VISIBLE:
        log = !input
          ? `Confirm that '${itemName}' is not displayed ${log}`
          : `Confirm that '${input}' is not displayed in '${itemName}' ${log}`;
        break;
      case Action.EXPECT_TEXT:
        log = `Confirm that '${itemName}' is '${input}' ${log}`;
        break;
      case Action.WAIT:
        log = `Wait for '${itemName}' ${log}`;
        break;
      case Action.GET_TEXT:
        log = `Get '${itemName}' ${log}`;
        break;
      case Action.NONE:
        log += input;
        break;
  ...
  format_ja(pageName: string, itemName: string, action: Action, input?: string): string {
    ...
    switch (action) {
      ...
      case Action.EXPECT_NOT_VISIBLE:
        log += !input
          ? `「${itemName}」が表示されないことを確認する。`
          : `「${itemName}」に「${input}」が表示されないことを確認する。`;
        break;
      case Action.EXPECT_TEXT:
        log += `「${itemName}」が「${input}」であることを確認する。`;
        break;
      case Action.WAIT:
        log = `「${itemName}」待機する。 ${log}`;
        break;
      case Action.GET_TEXT:
        log = `「${itemName}」の値を取得する。 ${log}`;
        break;
      case Action.NONE:
        log += input;
        break;
